...
<?php
   ...
   //2. check the user with current id_admin and password exist (i can take off this etape if negle3 current password nkhali ghir new w confirm)
$sql= "SELECT * FROM tbl_admin WHERE id_admin=$id_admin AND password='$mot_de_passe_courrant'"  ;
//execute
...
if($nouveau_mot_de_passe==$confirmer){
    //update
    //echo "password match";
    $sql2 = "UPDATE tbl_admin SET password='$nouveau_mot_de_passe' WHERE id_admin=$id_admin";
    $res2 = mysqli_query($conn, $sql2);
    if($res2==TRUE)
    {
        $_SESSION['changer-mdp']="<div class='success'>le mot de psse a été modifier avec successé</div>";
        header('location:'.SITEURL.'admin/manage-admin.php');
       
    }
    else{
        //dislpay error
        $_SESSION['changer-mdp']="<div class='error'>erreur lors du changement du mot de passe</div>";
        header('location:'.SITEURL.'admin/manage-admin.php');
    ...
    $_SESSION['pwd-not-matched']="<div class='error'>matcher ton mot de passe avec le confirmer</div>";
    header('location:'.SITEURL.'admin/manage-admin.php');
